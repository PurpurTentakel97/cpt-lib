...
TEST(Optional, OK) {
    cpt::Optional<Entry> constexpr optional = Entry::default_entry;
...
TEST(Optional, Empty) {
    cpt::Optional<Entry> constexpr optional = {};

    ASSERT_FALSE(optional.ok());
    ASSERT_THROW([optional]() { [[maybe_unused]] auto const o = optional.unwrap(); }(), cpt::BadOptionalAccess);
